(() => {
  ...
  const dismissToast = (toast, source = "default") => {
    ...
    const onDismiss = toast._onDismiss;
    toast._onDismiss = null;
    if (typeof onDismiss === "function") {
      try {
        onDismiss(toast, source);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  const showSiteToast = ({
    ...
    if (existingToast && existingToast.isConnected && existingToast.dataset.closing !== "true") {
      ...
      existingToast._onDismiss = onDismiss || existingToast._onDismiss;
