...
(() => {
  ...
  const fetchRepositoryUpdatedAt = async () => {
    ...
    try {
      ...
      if (error?.name === "AbortError") {
        console.warn(`Latest commit timestamp request timed out after ${API_TIMEOUT_MS}ms.`);
      } else {
  ...
  const queuePageUpdateToast = async (activeCatalog = {}) => {
    const updateToast = getUpdateToastConfig(activeCatalog);
    if (updateToast.enabled === false) return;
    ...
    if (!formattedTimestamp && !relativeTimestamp) return;
  ...
  onReady(async () => {
    try {
      ...
      const activeCatalog =
        window.VOXLIS_PAGE?.catalog ??
        window.VOXLIS_CONFIG?.activeCatalogPage ??
        {};

      const pageTitle = activeCatalog.pageTitle
        ? `voxlis - ${activeCatalog.pageTitle}`
        : "voxlis - BETA";
      ...
      if (cardsMount) {
        ...
        // Non-blocking. If the API takes longer than 10s, it aborts and skips the toast.
